...
cols_angles = ['ORIENTATION X (pitch ø)', 'ORIENTATION Y (roll ø)',
               'ORIENTATION Z (yaw ø)']
...
accel = np.array([df['Acc x'],
                  df['Acc y'],
                  df['Acc z']])
grav = np.array([df['Gyro x'],
                 df['Gyro y'],
                 df['Gyro z']])
line = np.array([df['LINEAR ACCELERATION X (m/sý)'],
                 df['LINEAR ACCELERATION Y (m/sý)'],
                 df['LINEAR ACCELERATION Z (m/sý)']])

# Set up arrays to hold euler angles for rotation matrices
pitch = df['ORIENTATION X (pitch ø)']
roll = df['ORIENTATION Y (roll ø)']
yaw = df['ORIENTATION Z (yaw ø)']
...
for i in range(df.shape[0]):
    ...
    earth_accels[:, i] = R_z(yaw[i]) @ R_y(roll[i]
                                           ) @ R_x(pitch[i]) @ accel[:, i]
    earth_gravity[:, i] = R_z(
        yaw[i]) @ R_y(roll[i]) @ R_x(pitch[i]) @ grav[:, i]
...
cols_body = ['ACCELEROMETER X (m/sý)', 'ACCELEROMETER Y (m/sý)',
             'ACCELEROMETER Z (m/sý)', 'GRAVITY X (m/sý)', 'GRAVITY Y (m/sý)',
             'GRAVITY Z (m/sý)', 'LINEAR ACCELERATION X (m/sý)',
             'LINEAR ACCELERATION Y (m/sý)', 'LINEAR ACCELERATION Z (m/sý)',]
...
fig3.suptitle('3D Trajectory of ball', fontsize=20)
ax = plt.axes(projection='3d')
ax.plot3D(x, y, z, c='red', lw=5, label='ball trajectory')
...
ax.plot3D(x, y, z, 'k', lw=5, label='Attenuated ball trajectory')
ax.set_xlabel('X position (m)')
ax.set_ylabel('Y position (m)')
ax.set_zlabel('Z position (m)')
...
fig6.suptitle('Ball trajectory and pose', fontsize=20)
ax4 = plt.axes(projection='3d')
ax4.plot3D(x, y, z, 'k', lw=5, label='Attenuated ball trajectory')
